	...
	Properties
	{
		...
		SubShader
		{
			...
			void SurfaceReveal(Input input, inout SurfaceOutputStandard R)
			{
				...
				float  Strength = Scale - cos(_LightAngle * (UNITY_PI / 360.0));
				Strength = clamp(Strength * _StrengthScalar * _StrengthDistance * _RequirementsMet, 0, 1);
				fixed4 RC = tex2D(_MainTex, input.UVMainTex) * _Color;
				R.Albedo = RC.rgb * RC.a * Strength;
