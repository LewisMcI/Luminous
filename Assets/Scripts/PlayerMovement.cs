...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (x != 0 || z != 0)
        {
            if (playerAnim.GetBool("isWalking") != true)
            {
                GetComponent<PhotonView>().RPC("UpdateAnimations", RpcTarget.All, PhotonNetwork.IsMasterClient, true);
            }
        ...
        * Not upwards and downwards
        ...
        {
            if (playerAnim.GetBool("isWalking") != false)
            {
                GetComponent<PhotonView>().RPC("UpdateAnimations", RpcTarget.All, PhotonNetwork.IsMasterClient, false);
            }
        }
    }

    public void UpdateAnimations(string name, bool newState)
    {
        playerAnim.SetBool(name, newState); 
    }
}
